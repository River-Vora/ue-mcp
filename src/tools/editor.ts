...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    capture_screenshot: bp("Screenshot. target=pie synchronously captures the selected PIE client game viewport with UMG/Slate UI; target=editor captures the level viewport; target=window captures the selected PIE client Slate window when PIE is running, otherwise the active editor window. Multi-instance PIE automatically prefers a world with a game viewport; pass pieInstance or worldPath to select explicitly. Params: filename?, target? (auto|pie|editor|window), pieInstance?, worldPath?. Returns the resolved PIE instance/world and image dimensions (#226/#724).", "capture_screenshot", (p) => ({ filename: p.filename, resolution: p.resolution, target: p.target, pieInstance: p.pieInstance, worldPath: p.worldPath })),
  ...
  undefined,
  {
    ...
    target: z.string().optional().describe("capture_screenshot target: auto (default) | pie | editor | window"),
    pieInstance: z.number().int().optional().describe("capture_screenshot: select a specific PIE instance (for example 1 for UEDPIE_1)"),
