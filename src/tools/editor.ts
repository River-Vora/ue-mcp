...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    sample_bone_transforms: bp("Read live skeletal bone and socket transforms off an actor - the evidence an animation check is actually built on. Pass bones (bone OR socket names) or omit for every bone up to limit. space=world (default) or component; component space is independent of where the actor is standing. Also reports the AnimInstance class/path. Params: actorLabel, componentName?, bones?, space?, limit?, world? (editor|pie|auto), pieInstance? (#756/#757/#761/#764)", "sample_bone_transforms", (p) => ({ actorLabel: p.actorLabel, componentName: p.componentName, bones: p.bones, space: p.space, limit: p.limit, world: p.world, pieInstance: p.pieInstance })),
  ...
  undefined,
  {
    ...
    space: z.string().optional().describe("sample_bone_transforms: world (default) | component (#756)"),
