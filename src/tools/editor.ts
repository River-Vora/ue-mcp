...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    capture_screenshot: bp("Screenshot. target=pie captures the actual PIE game viewport with UI + on-screen debug canvas (what the player sees), even in Play-in-New-Window; target=editor captures the level viewport. Params: filename?, resolution?, target? (auto|pie|editor; auto routes to PIE when running). Returns includesDebugCanvas (#226/#724)", "capture_screenshot"),
