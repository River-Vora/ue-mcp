...
export const editorTool: ToolDef = categoryTool(
  ...
  "Editor commands, Python execution, PIE, undo/redo, hot reload, viewport, performance, sequencer, build pipeline, logs, editor control.",
  {
    ...
    get_message_log: bp("Read a Message Log listing (BlueprintLog, MapCheck, AssetCheck, PIE, LoadErrors, LightingResults). Returns messages with severity plus whole-listing total/errors/warnings counts, so a truncated read still tells you honestly whether the log is clean. An unknown logName is an error, not an empty log. Params: logName? (default BlueprintLog), maxLines? (default 200), filter? (severity substring)", "get_message_log"),
