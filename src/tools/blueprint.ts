...
export const blueprintTool: ToolDef = categoryTool(
  ...
  "Blueprint reading, authoring, and compilation, including AnimGraph/EventGraph node scripting: add_node, connect_pins, search_node_types, plus variables, functions, graphs, components, interfaces, and event dispatchers.",
  {
    ...
    list_variables:    bp("List variables with name, type, guid, category, tooltip, exposeOnSpawn, blueprintReadOnly, and editFlag ('EditAnywhere'|'EditDefaultsOnly'|'EditInstanceOnly'|'none'). instanceEditable is true only for EditAnywhere - EditDefaultsOnly is class-defaults-only and reports false (#744). Params: assetPath", "list_blueprint_variables", (p) => ({ path: p.assetPath })),
    ...
    set_variable_properties: bp("Edit variable properties. Note: replication is set with networking(set_property_replicated), and blueprintReadOnly is not writable here - list_variables reports it. Params: assetPath, name, instanceEditable?, category?, tooltip?, exposeOnSpawn?", "set_variable_properties", (p) => ({ path: p.assetPath, name: p.name, instanceEditable: p.instanceEditable, category: p.category, tooltip: p.tooltip, exposeOnSpawn: p.exposeOnSpawn })),
