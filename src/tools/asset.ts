...
export const assetTool: ToolDef = categoryTool(
  ...
  undefined,
  {
    save: z.boolean().optional().describe("set_property / append_array_elements / create_subobject / import_stringtable_csv / bulk_set_properties / bulk_upsert_data_assets / import_texture_batch / set_mesh_materials_batch: write the changed package to disk (default true). false leaves the change in memory only and reports persisted=false with the reason (#931)."),
