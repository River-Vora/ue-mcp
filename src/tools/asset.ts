...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    bulk_set_properties:  bp("Set dotted UPROPERTY paths on as many as 500 assets in one atomic-preflight batch. Every asset, path, and value is validated before mutation. Returns per-property readback, counts, targeted save results, and a replayable rollback payload. Params: items ([{assetPath, properties}]), save? (default true), dryRun? (default false)", "bulk_set_asset_properties", (p) => ({ items: p.items, save: p.save, dryRun: p.dryRun })),
  ...
  undefined,
  {
    ...
    ])).min(1).max(500).optional().describe("import_texture_batch entries, or bulk_set_properties entries [{assetPath, properties}] (max 500)"),
    save: z.boolean().optional().describe("import_texture_batch/bulk_set_properties: save changed packages immediately (default true)"),
    ...
    dryRun: z.boolean().optional().describe("migrate/bulk_set_properties: validate and report without mutation"),
