...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    create_render_target_2d: bp("Create and persist a TextureRenderTarget2D asset. Render format is applied before resource initialization. Params: name, packagePath? (default /Game), width? (default 512), height? (default 512), format? (R8|RG8|RGBA8|RGBA8_SRGB|R16F|RG16F|RGBA16F|R32F|RG32F|RGBA32F|RGB10A2, default RGBA8_SRGB), clearColor? ({r,g,b,a}, default transparent), generateMips? (default false), targetGamma? (default 0), onConflict? (skip|error)", "create_render_target_2d", (p) => ({ name: p.name, packagePath: p.packagePath, width: p.width, height: p.height, format: p.format, clearColor: p.clearColor, generateMips: p.generateMips, targetGamma: p.targetGamma, onConflict: p.onConflict })),
  ...
  undefined,
  {
    ...
    width: z.number().int().min(1).max(16384).optional().describe("create_render_target_2d: pixel width (default 512)"),
    height: z.number().int().min(1).max(16384).optional().describe("create_render_target_2d: pixel height (default 512)"),
    clearColor: z.object({
      r: z.number().optional(),
      g: z.number().optional(),
      b: z.number().optional(),
