...
export const assetTool: ToolDef = categoryTool(
  ...
  undefined,
  {
    ...
    dryRun: z.boolean().optional().describe("migrate: resolve and report without copying (#760). bulk_upsert_data_assets: run the full preflight and report planned statuses without writing. fixup_redirectors: report the redirectors, referencers and packages it would load and save, and stop there (#908)"),
    ...
    replaceExisting: z.boolean().optional().describe("import_skeletal_mesh: replace an existing asset at the destination (default true) (#678). import_stringtable_csv: prune entries the CSV no longer carries, making the CSV canonical rather than additive (default false) (#978)."),
    ...
    lodIndex: z.number().int().min(0).optional().describe("get_mesh_geometry / measure_mesh_geometry: which LOD to read (default 0)"),
