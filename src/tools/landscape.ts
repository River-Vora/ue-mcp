...
export const landscapeTool: ToolDef = categoryTool(
  ...
  "Landscape terrain: info, layers, sculpting, weight painting, materials, splines, proxies.",
  {
    ...
    refresh_physical_material_collision: {
      ...bp("UE 5.8+: refresh physical-material collision data on loaded World Partition LandscapeStreamingProxy actors after a LayerInfo PhysMaterial change. Filters combine: actorLabels[], guids[], and bounds {min,max}; omitting them targets every loaded proxy up to maxActors (default 256, hard max 1024). Unloaded proxies are untouched; pin them first with level(load_actor_descs). Refuses PIE/SIE and non-World-Partition maps. save defaults false; when true, only the matched external actor packages are targeted. Returns loaded/matched/refreshed/saved/failed counts and exact package paths.", "refresh_landscape_physical_material_collision", (p) => ({ actorLabels: p.actorLabels, guids: p.guids, bounds: p.bounds, maxActors: p.maxActors, save: p.save })),
      timeoutMs: 300_000,
    },
  },
  undefined,
  {
    ...
    save: z.boolean().optional().describe("refresh_physical_material_collision: save only affected external actor packages (default false)"),
  },
);
