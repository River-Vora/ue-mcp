...
export const niagaraTool: ToolDef = categoryTool(
  ...
  "Niagara VFX: systems, emitters, spawning, parameters, and graph authoring.",
  {
    ...
    list_module_inputs:  bp("List an emitter's modules with the inputs you can actually set - Spawn Rate, Lifetime, Colour, Sprite Size - each with its type and CURRENT value, read back through the same override map set_module_input writes to. Compile-time switches and enums are reported separately under switchPins rather than being conflated with values (#784). Params: systemPath, emitterName?, emitterIndex?, stackContext? (ParticleSpawn|ParticleUpdate|EmitterSpawn|EmitterUpdate|all - default all), moduleName?", "list_niagara_module_inputs", (p) => ({ systemPath: p.systemPath, emitterName: p.emitterName, emitterIndex: p.emitterIndex, stackContext: p.stackContext, moduleName: p.moduleName })),
    set_module_input:    bp("Set a module input value. Override-map-bound inputs (the numeric/colour values that matter) are written through the stack override map, the same path the Niagara stack editor uses; others fall back to the pin default. Reports writePath ('overrideMap'|'pinDefault') and the real previousValue so a caller can tell which happened and roll back (#769). Params: systemPath, moduleName, inputName, value, emitterName?, emitterIndex?, stackContext?", "set_niagara_module_input", (p) => ({ systemPath: p.systemPath, moduleName: p.moduleName, inputName: p.inputName, value: p.value, emitterName: p.emitterName, emitterIndex: p.emitterIndex, stackContext: p.stackContext })),
