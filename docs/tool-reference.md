...
| `bulk_set_properties` | Set dotted UPROPERTY paths on as many as 500 assets in one atomic-preflight batch. Every asset, path, and value is validated before mutation. Returns per-property readback, counts, targeted save results, and a replayable rollback payload. Params: `items ([{assetPath, properties}]), save? (default true), dryRun? (default false)` |
