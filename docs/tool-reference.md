...
## Reading and writing TMap properties

A property read hands back two forms: `value`, which is structured JSON, and `valueText`, which is Unreal export text. **Write `value` back, not `valueText`.**

Unreal exports map pairs as `((Key, Value),(Key2, Value2))`. When the key is a struct, that text does not read back through the engine's own importer, so copying `valueText` into a setter used to store zero pairs. Read handlers now report `valueTextRoundTrips` for any map-bearing property, and `mapPairCount` alongside it, so the lossy case is visible before you write.

Setters take a map in either of two shapes:

```jsonc
// Text keys (FName, FString, numbers, enums, gameplay tags)
{ "Damage.Fire": 12, "Damage.Ice": 8 }

// Any key, struct keys included. This is what a read returns for them.
[
  { "key": { "Tag": "Motion.Walk", "Slot": 1 }, "value": "/Game/Anim/Walk.Walk" },
  { "key": { "Tag": "Motion.Run",  "Slot": 2 }, "value": "/Game/Anim/Run.Run" }
]
```

Both shapes are applied entry by entry onto the key and value properties, never through text. A write that cannot store every entry it was given - a duplicate key, a key that does not resolve, a pair that fails to convert - **fails and leaves the previous value exactly as it was**. Successful container writes report `elementCount` (or `mapPairCount` when the map sits inside a struct), so a caller can confirm what landed without a second read.
...
| `get_object_properties` | Read reflected properties off any UObject, with the same targeting as invoke_object_function. Pass propertyNames to filter; names that do not exist come back under missingProperties instead of silently returning nothing. Properties holding a TMap are also reported under `values` in the structured form set_property accepts, because export text cannot carry a struct-keyed map back (#820). Params: `objectPath? \| target?, subsystemClass?, playerIndex?, propertyNames?, world? (editor\|pie\|auto), pieInstance? (#739)` |
