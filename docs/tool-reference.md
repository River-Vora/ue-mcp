...
| `inspect_runtime_instances` | Inspect every matching live widget instance in deterministic path order, including owning-player identity and selected reflected properties on the widget or subtree. Never silently selects the first class match. Requires a running PIE/Game world and errors instead of falling back to the editor world. Passing `childName` or `childClassFilter` implies `includeSubtree`. Provide `widgetName` or `classFilter`. See [Runtime widget inspection](runtime-widget-inspection.md). Params: `widgetName?, classFilter?, propertyNames[]?, includeSubtree?, childName?, childClassFilter?, viewportOnly?, world?, pieInstance?, maxInstances?, maxNodesPerInstance?` |
...
| `play_in_editor_ignore_blueprint_errors` | Start PIE for one launch with the editor's unresolved-Blueprint-error prompt suppressed. PIE then runs whatever bytecode those Blueprints last compiled to, so every call is authorized: set `pie.allowIgnoreBlueprintErrors: true` in the [`ue-mcp:` config block](config-file.md#ue-mcp-block-reference) to pre-authorize it, otherwise the user answers an MCP approval prompt. Blueprints PIE would have to recompile first (dirty non-data Blueprints, errored Level Blueprints) are refused with `code: blueprints_require_compile` and the offending paths. On success the response names every suppressed Blueprint in `loadedErroredBlueprints`, and the warning flags are restored once the launch is processed. Params: `waitForAssetRegistry? (default true), assetRegistryTimeoutSeconds? (default 180)` |
...
| `read_bone_transforms` | Read live skeletal bone and socket transforms off an actor, once. This is a point-in-time read, NOT a time series - for per-frame capture over a window use the pie category's observe actions. Pass bones (bone OR socket names) or omit for every bone up to limit. space=world (default) or component; component space is independent of where the actor is standing. Pass relativeTo (bone OR socket name) to express every sample in that live reference frame; relativeTo supersedes space and is calculated in component space. An unknown relativeTo fails the call. Also reports the AnimInstance class/path. Params: `actorLabel, componentName?, bones?, relativeTo?, space?, limit?, world? (editor\|pie\|auto), pieInstance? (#756/#757/#761/#764)` |
