...
| `set_variable_properties` | Edit variable properties. Note: replication is set with networking(set_property_replicated), and blueprintReadOnly is not writable here - list_variables reports it. Params: `assetPath, name, instanceEditable?, category?, tooltip?, exposeOnSpawn?` |
