...
UE-MCP exposes **<!-- count:tools -->24<!-- /count --> category tools** covering **<!-- count:actions -->791+<!-- /count --> actions**, plus a `flow` tool for running multi-step YAML workflows. Every category tool takes an `action` parameter that selects the operation, plus action-specific parameters.
...
| `move_bt_node` | Reconnect a node to a different parent, reorder it among its siblings, or both, then recompile. The whole subtree travels with it, and a move under its own descendant is refused. Params: `assetPath, node, parent?, index? (#947)` |
