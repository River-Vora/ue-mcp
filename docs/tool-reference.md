...
| `refresh_physical_material_collision` | UE 5.8+: refresh physical-material collision data on loaded World Partition LandscapeStreamingProxy actors after a LayerInfo PhysMaterial change. Filters combine: actorLabels[], guids[], and bounds {min,max}; omitting them targets every loaded proxy up to maxActors (default 256, hard max 1024). Unloaded proxies are untouched; pin them first with level(load_actor_descs). Refuses PIE/SIE and non-World-Partition maps. save defaults false; when true, only the matched external actor packages are targeted. Returns loaded/matched/refreshed/saved/failed counts and exact package paths |
