...
| `nudge_component` | Make small component adjustments in world, actor, parent, or component space. Translation uses forward/right/up centimetres; rotation uses a quaternion axis-angle delta so attached or socketed components avoid raw relative-Euler drift. For visual requests, resolve the target, reference frame, viewpoint, operation, amount, and constraints; character left/right is actor-relative, while clockwise/counterclockwise requires a viewpoint. For character body attachments, stop actor/root movement and freeze the parent skeletal pose before the baseline, then hold both across every probe and capture. Verify with a front-lit close packet (front/side/top/three-quarter) where the contact region fills roughly 50-75% of the frame; reject dark or wide-only evidence. Params: `actorLabel, componentName, frame?, translationDelta?, axisRotation?, scaleMultiplier?, world?, pieInstance?` |
