...
| `play_in_editor_ignore_blueprint_errors` | Start PIE while suppressing unresolved Blueprint compiler-error dialogs for this launch only. This can run stale or invalid Blueprint bytecode, so it requires either an MCP user-approval prompt or the exact marker `UE_MCP_ALLOW_IGNORE_BLUEPRINT_ERRORS=true` in `AGENTS.md`, `CLAUDE.md`, or `.codex/instructions.md` at/above the project directory. Dirty or errored Level Blueprints that PIE must recompile are refused. Params: `waitForAssetRegistry? (default true), assetRegistryTimeoutSeconds? (default 180)` |
